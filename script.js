    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('printBillIncomeBtn').addEventListener('click', function() {
        ...
        window.getBillIncomes().forEach(b => {
          const subtotal = b.type * b.quantity;
          total += subtotal;
          html += `<tr><td>${sanitizeHTML(window.getBillLabel(b.type))}</td><td>${sanitizeHTML(b.quantity)}</td><td>${sanitizeHTML(window.getBillLabel(b.type))}</td><td>${window.getBillLabel(subtotal)}</td></tr>`;
        });
