    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('printBillIncomeBtn').addEventListener('click', function() {
        ...
        window.getBillIncomes().forEach(b => {
          const subtotal = b.type * b.quantity;
          total += subtotal;
          html += `<tr><td>${sanitizeHTML(window.getBillLabel(b.type))}</td><td>${b.quantity}</td><td>${sanitizeHTML(window.getBillLabel(b.type))}</td><td>${sanitizeHTML(window.getBillLabel(subtotal))}</td></tr>`;
        });
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('printCoinIncomeBtn').addEventListener('click', function() {
        ...
        coinIncomes.forEach(c => {
          ...
          const subtotal = value * c.quantity;
          total += subtotal;
          html += `<tr><td>${sanitizeHTML(coinLabels[c.type])}</td><td>${c.quantity}</td><td>${sanitizeHTML(coinLabels[c.type])}</td><td>$${sanitizeHTML(formatNumberWithDots(subtotal))}</td></tr>`;
        });
      ...
      document.getElementById('printExpensesBtn').addEventListener('click', function() {
        ...
        let total = 0;
        expenses.forEach(e => {
          html += `<tr><td>${sanitizeHTML(e.date)}</td><td>${sanitizeHTML(e.category)}</td><td>${sanitizeHTML(e.description)}</td><td>$${sanitizeHTML(formatNumberWithDots(e.amount))}</td></tr>`;
          total += e.amount;
        });
        html += `<tr><th colspan='3' style='text-align:right;'>Total general:</th><th>$${sanitizeHTML(formatNumberWithDots(total))}</th></tr>`;
        html += `</table>`;
