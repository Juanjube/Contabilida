    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('printBillIncomeBtn').addEventListener('click', function() {
        ...
        window.getBillIncomes().forEach(b => {
          const subtotal = b.type * b.quantity;
          total += subtotal;
          html += `<tr><td>${window.getBillLabel(b.type)}</td><td>${escapeHTML(String(b.quantity))}</td><td>${window.getBillLabel(b.type)}</td><td>${window.getBillLabel(subtotal)}</td></tr>`;
        });
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      document.getElementById('printCoinIncomeBtn').addEventListener('click', function() {
        ...
        coinIncomes.forEach(c => {
          ...
          const subtotal = value * c.quantity;
          total += subtotal;
          html += `<tr><td>${escapeHTML(coinLabels[c.type] || c.type)}</td><td>${escapeHTML(String(c.quantity))}</td><td>${escapeHTML(coinLabels[c.type] || c.type)}</td><td>$${formatNumberWithDots(subtotal)}</td></tr>`;
        });
